...
var Blackbag = function(config) {
	...
	var process_data = function(db, exporter, queries, map) {
		...
		for(var query in queries) {
			...
			call_exporter(query_config, exporter, db, function(exported_data, query_continue) {
				console.log('exporter done', exported_data)
				if(!exported_data.query_continue) {
					...
					db.query(query_config, function(result, query_config) {
						// Query Returned
						console.log(result);
	...
	var load_ext_formatter = function(path) {
		...
		if(fs.existsSync(path)) {
			...
			console.log('Formatter not found!' + path);
			ext = {
				config : {
					options : {
	...
	var call_exporter = function(query_obj, exporter, db, cb) {
		var return_data = {
			query_continue : (exporter && exporter.config.options.query_continue == true) ? true : false,
			result : false
		};
