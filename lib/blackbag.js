...
var Blackbag = function(config) {
	...
	var save_data = function(data, query_obj) {
		...
		for(var d in data) {
			...
			var target_name = process_filename(filename, data[d]) + '.' + get_file_extension(format);
	...
	// Pulls the right file extension from the format string
	var get_file_extension = function(format) {
		var ext;
		switch(format) {
			case 'json':
				ext = 'json';
			...
			break;
			default:
				console.log("Couldn't find an extension for that format.");
			break;
		}
		return ext;
	};
