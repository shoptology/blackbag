...
var Blackbag = function(config) {
	...
	var init = function() {
		...
		if(config.options.format) {
			config = use_format_config(config.options.format, config);
		}
	...
	var get_data = function(exporter, queries, map) {
		...
		for(var query in queries) {
			...
			console.log('query_config', query_config);

			// Add in config values for the format used
			if(query_config.options.format) {
				query_config = use_format_config(query_config.options.format, query_config);
			}
	...
	var use_format_config = function(format, config) {
		
		var path = 'config/formats/'+format+'.js';
		console.log('Checking for Format config', path);
		if (fs.existsSync(path)) {
			console.log('File Exists!');
			var ext_config = require('../config/formats/'+format+'.js');
