...
var Blackbag = function(config) {
	...
	var init = function() {
		...
		get_data(db, config.options.exporter, config.exports, config.options.map);
	};
	 ...
	 */
	var get_data = function(db, exporter, queries, map) {
		var queries_data = {
			total : queries.length,
			complete : 0
		};

		// Do we want to terminate if queries.length == 0?
		...
		for(var query in queries) {
			...
			call_exporter(query_config, exporter, db, function(exported_data) {
				if(!exported_data.query_continue) {
					// Query Returned
	...
	var call_exporter = function(query_obj, exporter, db, cb) {
		...
		if(exporter === false) {
			return_data.query_contiune = true;
			cb(return_data);
		} else {
			exporter.export(query_obj, db, function(data) {
